...
name: kit-zyb-docs
version: 1.0.0
description: 该技能用于获取帮帮文档（docs.yukework.com）内容，支持自动登录、Cookie管理、文档导出和内容提取功能。
---
...
1. **Cookie管理**：加密存储docs.yukework.com的Cookie到本地文件，使用Fernet对称加密
...
1. 从本地加密文件中获取docs.yukework.com的Cookie（自动解密）
...
### 创建导出任务
- **接口地址**：`https://docs.yukework.com/document-application/api/v2/file/export`
- **请求方法**：POST
...
### 获取导出进度
- **接口地址**：`https://docs.yukework.com/document-application/api/v2/file/export/progress?taskId={taskId}`
- **请求方法**：GET
- **请求头**：需要包含Cookie
- **状态判断**：
...
从文档链接中提取fileId：
- 文档链接格式：`https://docs.yukework.com/doc?fileId={fileId}`
...
```bash
# 获取文档内容（仅显示预览，不保存文件）
python scripts/fetch_bangbang_doc.py --url "https://docs.yukework.com/doc?fileId=2003342689400123394"
...
```bash
python scripts/cookie_manager.py --action get --domain docs.yukework.com
python scripts/cookie_manager.py --action save --domain docs.yukework.com --cookie "your_cookie_here"
```
...
```bash
# 测试登录功能
python scripts/browser_automation.py "https://docs.yukework.com/doc?fileId=2003342689400123394" --verbose
```
